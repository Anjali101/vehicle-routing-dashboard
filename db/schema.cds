...
entity Route: cuid {
  ...
  to_Customers: association to many Customer on to_Customers.route_id = route_id;
  to_BlockedRoads: association to many BlockedRoad on to_BlockedRoads.route_id = route_id;
  to_Constraints: association to many Constraints on to_Constraints.route_id = route_id;
}
...
entity RouteClassification as select from Route {
  key route_id                           as RouteID,
      route_code                         as RouteCode,
      route_date                         as RouteDate,

      count(distinct to_Constraints.ID)  as NumberOfConstraints : Integer,

      max(to_Customers.customer_time_window_to_min - to_Customers.customer_time_window_from_min)
    - min(to_Customers.customer_time_window_to_min - to_Customers.customer_time_window_from_min)
                                          as TimeWindowRange : Integer
